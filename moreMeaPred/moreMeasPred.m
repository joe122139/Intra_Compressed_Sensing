...
img_ed =1;
    ...
    for image= img_st:1:img_ed
        ...
        for per_oneInRow =one_per%-0.54:0.1:0.8%0.0074   %0.0074;  %0.04305 %0.03:0.05:0.03;
            ...
            mid_val=128;
            [org_M modi_M morePattern_M]=genMatrix(N,per_oneInRow,filename,id_bottom_r,id_right_c,mat_type,isRand);
        ...
        %    co(2) = coherence(modi_M,n)
          ...
          for Q_step = Q_step_st:inc:n_Q_step;
               ...
               %                
                            ...
                            imageName = sprintf('./image/reconImage/%s/%d/SR%.2fN%dQ%d_sel%d%s%s_pdct%d_lowF%d_%sMorePred.tif',...
                                vname,N,SR(sr_idx),N,Q_step,selM,rm,ctwt,pDCT,takeLowF,dctMat_type);
                      
                            tic
                     %       [MSE(:,:,:,:,f),bit_per_F,PSNR(selM,f,Q),coff_ary(:,:,selM,f),SSIM(selM,f,Q)] = moreMeasPred_1F(A,SR(sr_idx),N,sensM,psi,mode,selM,Q_step,pred_method,recon_method,imageName,dctdwt,id_bottom_r,id_right_c,takeLowF,isPrint);
                            [MSE(:,:,:,:,f),bit_per_F,PSNR(selM,f,Q),coff_ary(:,:,selM,f),SSIM(selM,f,Q)] = moreMeasPred_1F(A,SR(sr_idx),N,sensM,psi,mode,selM,Q_step,pred_method,recon_method,imageName,dctdwt,id_bottom_r,id_right_c,takeLowF,0);
                     toc
            ...
            if(pDCT)
                ...
                data_dir= 'paper_data/moreMeasPred/';
            end
